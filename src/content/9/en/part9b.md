...
As you can see, this is still ordinary basic JavaScript with no additional TS features. It compiles and runs with  <i>npm run ts-node -- -O '{""noImplicitAny"": false}' .\multiplier.ts</i>, as it would with Node. We have to specify <i>noImplicitAny</i> as the default installation of <i>ts-node</i> woudl fail to compile.
